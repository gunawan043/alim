...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        // ASSET INTELLIGENCE
        $schedule->command('sarpras:repair-vs-replace --all')
            ->dailyAt('02:30')
            ->withoutOverlapping()
            ->runInBackground();

        $schedule->command('sarpras:recompute-health')
            ->dailyAt('01:30')
            ->withoutOverlapping()
            ->runInBackground();
        ...
        // UKS AUTO IMMUNIZATION
        // Jalankan setiap hari jam 2 pagi — buat jadwal imunisasi berdasarkan usia siswa
        $schedule->command('uks:auto-immunization')
            ->dailyAt('02:00')
            ->withoutOverlapping()
            ->runInBackground();
    }
