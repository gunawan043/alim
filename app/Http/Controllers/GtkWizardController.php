...
class GtkWizardController extends Controller
{
        ...
        try {
            ...
            return response()->json([
                ...
                'data'    => [
                    ...
                    'name'     => $user->name,
                    'email'    => $user->email,
                    'password' => $this->defaultPassword($data['employment']['nupy']),
                ],
            ], 201);
            ...
            DB::rollBack();
            \Log::error('GtkWizardController@store: ' . $e->getMessage(), [
    ...
    public function update(Request $request, string $userId, string $uuid): JsonResponse
    {
        try {
            ...
            DB::rollBack();
            \Log::error('GtkWizardController@update: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
            return response()->json([
                'success' => false,
    ...
    public function destroy(string $userId, string $uuid): JsonResponse
    {
        try {
            ...
            \Log::error('GtkWizardController@destroy: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function storeFamilyMember(Request $request, string $userId, string $uuid): JsonResponse
    {
        ...
        try {
            ...
            DB::rollBack();
            \Log::error('storeFamilyMember: ' . $e->getMessage());
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($s) {
                $q->where('name', 'like', "%{$s}%")
                  ...
                  ->orWhereHas('gtkProfile.addresses', fn ($q) =>
                        $q->where('jalan', 'like', "%{$s}%")
                          ->orWhere('desa', 'like', "%{$s}%")
    ...
    {
        $baseQuery = fn () => User::whereHas('employment');
    ...
    {
        if (empty($employmentData)) return;
     ...
     */
    private function resolveJenisGtkId(string|null $value): ?string
    {
     ...
     */
    private function resolveJenisGtkName(string|null $value): ?string
    {
        if (!$value) return null;
     ...
     */
    private function resolveSchoolId(string|null $workUnitId): ?string
    {
