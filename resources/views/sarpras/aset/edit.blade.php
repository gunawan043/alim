...
@section('content')
    ...
    <form method="POST" action="{{ route('user.sarpras.aset.update', ['userId' => $userId, 'id' => $aset->id]) }}">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header"><h5 class="mb-0">Informasi Aset</h5></div>
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="room_id" id="room_id" class="form-control" required>
                                    ...
                                    @foreach($rooms as $r)
                                        <option value="{{ $r->id }}" {{ old('room_id', $aset->room_id) == $r->id ? 'selected' : '' }}>{{ $r->room_name }} ({{ $r->room_type }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <select name="asset_category_id" id="asset_category_id" class="form-control" required>
                                        ...
                                        @foreach($categories as $c)
                                            <option value="{{ $c->id }}" {{ old('asset_category_id', $aset->asset_category_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="asset_name" class="form-control" value="{{ old('asset_name', $aset->asset_name) }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Kode Aset</label>
                                <input type="text" name="asset_code" class="form-control" value="{{ old('asset_code', $aset->asset_code) }}" maxlength="50">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Merk</label>
                                <input type="text" name="brand" class="form-control" value="{{ old('brand', $aset->brand) }}" maxlength="100">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Model / Tipe</label>
                                <input type="text" name="model" class="form-control" value="{{ old('model', $aset->model) }}" maxlength="100">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Nomor Seri</label>
                                <input type="text" name="serial_number" class="form-control" value="{{ old('serial_number', $aset->serial_number) }}" maxlength="100">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Warna</label>
                                <input type="text" name="color" class="form-control" value="{{ old('color', $aset->color) }}" maxlength="50">
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="condition" class="form-control" required>
                                    ...
                                    @foreach(App\Models\Asset::CONDITION_OPTIONS as $c)
                                        <option value="{{ $c }}" {{ old('condition', $aset->condition) == $c ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $c)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="status" class="form-control">
                                    @foreach(App\Models\Asset::STATUS_OPTIONS as $s)
                                        <option value="{{ $s }}" {{ old('status', $aset->status) == $s ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $s)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="acquisition_source" class="form-control">
                                    ...
                                    @foreach(App\Models\Asset::ACQUISITION_SOURCE_OPTIONS as $src)
                                        <option value="{{ $src }}" {{ old('acquisition_source', $aset->acquisition_source) == $src ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $src)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Tahun Perolehan</label>
                                <input type="number" name="acquisition_year" class="form-control" value="{{ old('acquisition_year', $aset->acquisition_year) }}" min="1900" max="2100">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Harga Perolehan (Rp)</label>
                                <input type="number" name="acquisition_price" class="form-control" value="{{ old('acquisition_price', $aset->acquisition_price) }}" min="0">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Sumber Dana</label>
                                <input type="text" name="funding_source" class="form-control" value="{{ old('funding_source', $aset->funding_source) }}" maxlength="100">
                            </div>
                            <div class="col-12">
                                <label class="form-label">Spesifikasi</label>
                                <textarea name="specification" class="form-control" rows="2">{{ old('specification', $aset->specification) }}</textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label">Catatan</label>
                                <textarea name="notes" class="form-control" rows="2">{{ old('notes', $aset->notes) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_bookable" value="1" {{ old('is_bookable', $aset->is_bookable) ? 'checked' : '' }}>
                                <label class="form-check-label">Aset dapat dipinjam</label>
                            </div>
                        </div>
                        <hr>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $aset->is_active) ? 'checked' : '' }}>
