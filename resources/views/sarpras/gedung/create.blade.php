...
@section('title') Tambah Gedung @endsection
...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('title') Tambah Gedung @endslot
    @endcomponent
    ...
    <form method="POST" action="{{ route('user.sarpras.gedung.store', ['userId' => $userId]) }}">
        @csrf
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="building_name" class="form-control" value="{{ old('building_name') }}" required placeholder="Contoh: Gedung A">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Kode Gedung</label>
                                <input type="text" name="building_code" class="form-control" value="{{ old('building_code') }}" placeholder="GDG-001" maxlength="30">
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="building_type" class="form-control" required>
                                    ...
                                    @foreach(App\Models\AssetBuilding::BUILDING_TYPE_OPTIONS as $t)
                                        <option value="{{ $t }}" {{ old('building_type') == $t ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $t)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Jumlah Lantai</label>
                                <input type="number" name="total_floors" class="form-control" value="{{ old('total_floors', 1) }}" min="1" max="20">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Luas (m²)</label>
                                <input type="number" name="building_area" class="form-control" value="{{ old('building_area') }}" min="0" step="0.1" placeholder="500">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Tahun Dibangun</label>
                                <input type="number" name="build_year" class="form-control" value="{{ old('build_year') }}" min="1900" max="2100" placeholder="2015">
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="structure_condition" class="form-control" required>
                                    ...
                                    @foreach(App\Models\AssetBuilding::CONDITION_OPTIONS as $c)
                                        <option value="{{ $c }}" {{ old('structure_condition') == $c ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $c)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="ownership_status" class="form-control">
                                    ...
                                    @foreach(App\Models\AssetBuilding::OWNERSHIP_OPTIONS as $o)
                                        <option value="{{ $o }}" {{ old('ownership_status') == $o ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $o)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Nomor IMB</label>
                                <input type="text" name="imb_number" class="form-control" value="{{ old('imb_number') }}" placeholder="12/IMB/2015">
                            </div>
                            <div class="col-12">
        ...
        <div class="d-flex justify-content-end gap-2 mt-3">
            <a href="{{ route('user.sarpras.gedung.index', ['userId' => $userId]) }}" class="btn btn-light">Batal</a>
