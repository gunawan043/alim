...
@php
    // Helper decode JSON -> newline string
    $decodeLines = function($val) {
    ...
    // Decode tahapan_seleksi
    $tahapanList = [];
    if (!empty($job->tahapan_seleksi)) {
        $tahapanList = is_array($job->tahapan_seleksi)
            ? $job->tahapan_seleksi
            : (json_decode($job->tahapan_seleksi, true) ?: []);
    }
    if (empty($tahapanList)) $tahapanList = ['Seleksi Administrasi', 'Tes Tertulis', 'Wawancara'];

    // Decode rentang_gaji
    $rgMin = old('rentang_gaji.min');
    $rgMax = old('rentang_gaji.max');
    if ($rgMin === null || $rgMax === null) {
        $rg = is_array($job->rentang_gaji) ? $job->rentang_gaji : (json_decode($job->rentang_gaji ?? '{}', true) ?: []);
        $rgMin = $rgMin ?? ($rg['min'] ?? '');
        $rgMax = $rgMax ?? ($rg['max'] ?? '');
    }
    $gajiAktif = old('_gaji_aktif', ($rgMin !== '' || $rgMax !== '') ? '1' : '0');
@endphp
...
<div class="card card-body mb-n3">
        ...
        <div class="row g-3">
                ...
                <select class="form-control @error('work_unit_id') is-invalid @enderror"
                    ...
                    @foreach ($workUnits as $unit)
                        <option value="{{ $unit->uuid }}"
                            {{ old('work_unit_id', $job->work_unit_id) == $unit->uuid ? 'selected' : '' }}>
                            {{ $unit->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-lg-6">
                ...
                <select class="form-control @error('jenis_pegawai') is-invalid @enderror"
                    data-choices name="jenis_pegawai">
                    <option value="">-- Pilih Jenis --</option>
                    @foreach(['pns'=>'PNS','pppk'=>'PPPK','honor'=>'Honor','kontrak'=>'Kontrak','magang'=>'Magang'] as $val => $label)
                        <option value="{{ $val }}" {{ old('jenis_pegawai', $job->jenis_pegawai) == $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-lg-6">
                ...
                <select class="form-control @error('status_pegawai') is-invalid @enderror"
                    ...
                    @foreach(['tetap'=>'Tetap','kontrak'=>'Kontrak','probation'=>'Probation'] as $val => $label)
                        <option value="{{ $val }}" {{ old('status_pegawai', $job->status_pegawai) == $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <input type="date" class="form-control @error('tanggal_mulai') is-invalid @enderror"
                    ...
                    value="{{ old('tanggal_mulai', $job->tanggal_mulai ? $job->tanggal_mulai->format('Y-m-d') : '') }}" required />
            ...
            <div class="col-md-3">
                ...
                <input type="date" class="form-control @error('tanggal_selesai') is-invalid @enderror"
                    ...
                    value="{{ old('tanggal_selesai', $job->tanggal_selesai ? $job->tanggal_selesai->format('Y-m-d') : '') }}" required />
    ...
    <div class="form-section" style="margin-top: 24px;">
        ...
        <input type="hidden" name="_gaji_aktif" id="_gaji_aktif" value="{{ $gajiAktif }}">

        <div id="gaji-section" class="{{ $gajiAktif == '1' ? 'show' : '' }}">
        ...
        <div class="row g-3">
            ...
            <div class="col-lg-6">
                ...
                <textarea class="form-control @error('kualifikasi_pendidikan') is-invalid @enderror"
                    ...
                    placeholder="- Minimal S1 Akuntansi&#10;- Diutamakan lulusan PTN">{{ old('kualifikasi_pendidikan', $decodeLines($job->kualifikasi_pendidikan)) }}</textarea>
            ...
            <div class="col-lg-6">
                ...
                <textarea class="form-control @error('kualifikasi_pengalaman') is-invalid @enderror"
                    ...
                    placeholder="- Minimal 2 tahun di bidang keuangan">{{ old('kualifikasi_pengalaman', $decodeLines($job->kualifikasi_pengalaman)) }}</textarea>
                <div class="form-hint"><i class="ri-information-line"></i> Satu poin per baris</div>
            ...
            <div class="col-lg-6">
                ...
                <textarea class="form-control @error('persyaratan_umum') is-invalid @enderror"
                    ...
                    placeholder="- WNI&#10;- Usia minimal 18 tahun&#10;- Pendidikan minimal S1">{{ old('persyaratan_umum', $decodeLines($job->persyaratan_umum)) }}</textarea>
                <div class="form-hint"><i class="ri-information-line"></i> Satu poin per baris</div>
            ...
            <div class="col-lg-6">
                ...
                <textarea class="form-control @error('persyaratan_khusus') is-invalid @enderror"
                    ...
                    placeholder="- Menguasai Ms. Office&#10;- Berpengalaman di bidang keuangan">{{ old('persyaratan_khusus', $decodeLines($job->persyaratan_khusus)) }}</textarea>
                <div class="form-hint"><i class="ri-information-line"></i> Satu poin per baris</div>
            ...
            <div class="col-lg-6">
                ...
                <textarea class="form-control @error('fasilitas') is-invalid @enderror"
                    ...
                    placeholder="- BPJS Kesehatan&#10;- Tunjangan Transportasi&#10;- Cuti Tahunan">{{ old('fasilitas', $decodeLines($job->fasilitas)) }}</textarea>
                <div class="form-hint"><i class="ri-information-line"></i> Satu poin per baris</div>
            ...
            <div class="col-lg-6">
                ...
                <select class="form-control @error('kompetensi_dibutuhkan') is-invalid @enderror"
                    ...
                    @php
                        $selectedKompetensi = old('kompetensi_dibutuhkan');
                        if (is_null($selectedKompetensi)) {
                            $decoded = $decodeLines($job->kompetensi_dibutuhkan);
                            $selectedKompetensi = is_array($decoded) ? $decoded : [];
                        } elseif (is_string($selectedKompetensi)) {
                            $selectedKompetensi = [$selectedKompetensi];
                        }
                    @endphp
                    @foreach ($selectedKompetensi as $skill)
    ...
    <div class="form-section" style="margin-top: 24px;">
        ...
        <div class="tahapan-list" id="tahapanList">
            @foreach ($tahapanList as $i => $tahap)
