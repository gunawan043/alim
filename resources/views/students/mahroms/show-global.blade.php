...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('li_4') Detail @endslot
        @slot('title') {{ $mahrom->name }} @endslot
    @endcomponent
    ...
    <div class="row">
        {{-- Left Column: Mahrom Photo + Quick Info --}}
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body text-center">
                    ...
                    <p class="text-muted mb-3">
                        ...
                        @if($mahrom->student)
                            dari <a href="{{ route('user.students.show', ['userId' => $userId, 'santriUuid' => $mahrom->student->id]) }}" class="text-decoration-none">{{ $mahrom->student->name }}</a>
                        @endif
                    </p>
                ...
                <div class="card-footer bg-transparent border-top">
                    <div class="d-flex gap-2 justify-content-center flex-wrap">
                        <a href="{{ route('user.students.mahroms.globalEdit', ['userId' => $userId, 'mahromUuid' => $mahrom->id]) }}"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row g-4">
                        ...
                        {{-- Student Section (Global: tampilkan ringkas saja, link ke detail Santri) --}}
                        @if($mahrom->student)
                        <div class="col-12">
                            <h6 class="text-uppercase text-muted fw-semibold mb-3 border-bottom pb-2 mt-3">
                                <i class="ri-user-follow-line me-1"></i>Santri yang Dihubungi
                            </h6>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Nama Santri</label>
                            <div class="fw-semibold">
                                <a href="{{ route('user.students.show', ['userId' => $userId, 'santriUuid' => $mahrom->student->id]) }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="fw-semibold">{{ $mahrom->student->nisn ?? '—' }}</div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Kelas Aktif</label>
                            <div class="fw-semibold">
                                @php
                                    $cls = $mahrom->student->currentClassHistory?->studyGroup?->gradeLevel?->name
                                        ?? $mahrom->student->currentClassHistory?->studyGroup?->name;
                                @endphp
                                {{ $cls ?: '—' }}
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="fw-semibold">
                                @php
                                    $room = $mahrom->student->activeDormitoryResident?->room;
                                @endphp
