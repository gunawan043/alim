        ...
        <div class="alert alert-danger d-flex align-items-center gap-2 mb-3" role="alert">
            ...
            <div>
                ...
                <br><span class="text-muted small">
                    Gunakan menu <a href="{{ route('user.student-move.index', ['userId' => $userId, 'study_group_id' => $studyGroup->id]) }}" class="alert-link">Pindahkan Santri</a>
    ...
    @if(session('import_result'))
        ...
        @if($result['created'] > 0)
            <div class="alert alert-success alert-dismissible fade show" role="alert">
                ...
                @if($totalDup) — {{ $totalDup }} duplikat dilewati.@endif
                @if($totalErr) — {{ $totalErr }} error.@endif
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
        @endif
        @if($totalDup)
        ...
        <div class="row g-3 mb-3">
                ...
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <div>
                                <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Kapasitas Rombel</p>
                                <h4 class="fw-bold ff-secondary mb-0">{{ $studyGroup->full_name }}</h4>
                            </div>
                            <span class="avatar-sm flex-shrink-0 rounded bg-primary-subtle">
                                <i class="bx bx-group text-primary"></i>
                            </span>
                        </div>
                        <div class="d-flex align-items-end gap-2 mb-1">
                            <h2 class="fw-bold ff-secondary mb-0">{{ number_format($inClass) }}</h2>
                            <span class="text-muted mb-1">/ {{ $capacity }} slot</span>
                        </div>
                        <div class="progress" style="height:8px;">
                            <div class="progress-bar {{ $quotaFilledPct > 100 ? 'bg-danger' : 'bg-primary' }}" style="width:{{ min(100, $quotaFilledPct) }}%"></div>
                        </div>
                        <div class="d-flex justify-content-between mt-1">
                            <small class="{{ $quotaRemaining > 0 ? 'text-success' : 'text-danger' }}" style="font-size:10px;">
                                <i class="ri-{{ $quotaRemaining > 0 ? 'checkbox-circle-fill' : 'error-warning-fill' }} me-1"></i>
                                {{ $quotaRemaining > 0 ? $quotaRemaining.' slot tersisa' : 'Kapasitas penuh' }}
                            </small>
                            <small class="text-muted" style="font-size:10px;">{{ $quotaFilledPct }}% terisi</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-4 col-md-6">
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Wali Kelas</p>
                            <span class="avatar-sm flex-shrink-0 rounded bg-warning-subtle">
                                <i class="bx bx-user-circle text-warning"></i>
                            </span>
                        </div>
                        <h4 class="fw-bold ff-secondary mb-1" style="font-size:1rem;">{{ $studyGroup->homeroomTeacher?->name ?? '-' }}</h4>
                        <hr class="my-2">
                        <div class="d-flex gap-3">
                            <div>
                                <p class="text-muted mb-0" style="font-size:10px;">Tingkat</p>
                                <p class="fw-semibold mb-0" style="font-size:.85rem;">{{ $studyGroup->gradeLevel?->name ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-muted mb-0" style="font-size:10px;">Ruang</p>
                                <p class="fw-semibold mb-0" style="font-size:.85rem;">{{ $studyGroup->room ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-4 col-md-6">
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Ringkasan</p>
                            <span class="avatar-sm flex-shrink-0 rounded bg-info-subtle">
                                <i class="bx bxs-chart text-info"></i>
                            </span>
                        </div>
                        <div class="row g-2">
                            <div class="col-6">
                                <div class="bg-primary-subtle rounded p-2 text-center">
                                    <h4 class="fw-bold ff-secondary mb-0 text-primary" style="font-size:1.2rem;">{{ number_format($inClass) }}</h4>
                                    <small class="text-muted" style="font-size:9px;">Total Santri</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-success-subtle rounded p-2 text-center">
                                    <h4 class="fw-bold ff-secondary mb-0 text-success" style="font-size:1.2rem;">{{ $quotaFilledPct }}%</h4>
                                    <small class="text-muted" style="font-size:9px;">Terisi</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-warning-subtle rounded p-2 text-center">
                                    <h4 class="fw-bold ff-secondary mb-0 text-warning" style="font-size:1.2rem;">{{ $studyGroup->gradeLevel?->name ?? '-' }}</h4>
                                    <small class="text-muted" style="font-size:9px;">Tingkat</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-secondary-subtle rounded p-2 text-center">
                                    <h4 class="fw-bold ff-secondary mb-0" style="font-size:1.2rem;">{{ $quotaRemaining > 0 ? $quotaRemaining : 'Penuh' }}</h4>
                                    <small class="text-muted" style="font-size:9px;">Sisa Slot</small>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Total Santri</p>
                            <span class="avatar-sm flex-shrink-0 rounded bg-primary-subtle">
                                <i class="bx bx-group text-primary"></i>
                            </span>
                        </div>
                        <h2 class="fw-bold ff-secondary mb-1">{{ number_format($totalAll) }}</h2>
                        <div class="d-flex flex-wrap gap-2 mb-1">
                            <span class="badge bg-success-subtle text-success" style="font-size:10px;">
                                <i class="ri-checkbox-circle-fill me-1"></i>{{ number_format($totalActive) }} Aktif
                            </span>
                            @if(($totalAll - $totalActive) > 0)
                            <span class="badge bg-secondary-subtle text-secondary" style="font-size:10px;">
                                {{ number_format($totalAll - $totalActive) }} Non-Aktif
                            </span>
                            @endif
                        </div>
                        @php $pctActive = $totalAll > 0 ? round($totalActive / $totalAll * 100) : 0; @endphp
                        <div class="progress" style="height:4px;">
                            <div class="progress-bar bg-success" style="width:{{ $pctActive }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Status Rombel</p>
                            <span class="avatar-sm flex-shrink-0 rounded bg-success-subtle">
                                <i class="bx bx-check-circle text-success"></i>
                            </span>
                        </div>
                        @php
                            $inRombel  = $byRombel['in_rombel'] ?? 0;
                            $unassigned = $byRombel['unassigned'] ?? 0;
                            $totalAssigned = $inRombel + $unassigned;
                            $pctInRombel = $totalAssigned > 0 ? round($inRombel / $totalAssigned * 100) : 0;
                        @endphp
                        <h2 class="fw-bold ff-secondary mb-1">{{ number_format($inRombel) }}</h2>
                        <div class="d-flex align-items-center justify-content-between small mb-1">
                            <span class="text-muted" style="font-size:10px;">Sudah di Rombel</span>
                            <span class="fw-semibold text-success" style="font-size:10px;">{{ $pctInRombel }}%</span>
                        </div>
                        <div class="progress" style="height:6px;">
                            <div class="progress-bar bg-success" style="width:{{ $pctInRombel }}%"></div>
                            <div class="progress-bar bg-danger" style="width:{{ 100 - $pctInRombel }}%"></div>
                        </div>
                        <div class="d-flex justify-content-between mt-1">
                            <small class="text-muted" style="font-size:10px;"><i class="ri-checkbox-circle-fill text-success me-1"></i>{{ number_format($inRombel) }} assigned</small>
                            @if($unassigned > 0)
                            <small class="text-danger" style="font-size:10px;"><i class="ri-error-warning-fill me-1"></i>{{ number_format($unassigned) }} belum</small>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-6">
                <div class="card card-animate h-100 border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <p class="text-uppercase fw-semibold text-muted mb-0" style="font-size:10px;letter-spacing:.05em;">Distribusi per Tingkat</p>
                            <span class="avatar-sm flex-shrink-0 rounded bg-info-subtle">
                                <i class="bx bxs-book-content text-info"></i>
                            </span>
                        </div>
                        @php $totalPerTingkat = $distribusiPerTingkat->sum('total'); @endphp
                        <h2 class="fw-bold ff-secondary mb-2">{{ number_format($totalPerTingkat) }} <small class="fw-normal text-muted" style="font-size:.7em;">Santri</small></h2>
                        <div class="d-flex flex-column gap-1">
                            @forelse($distribusiPerTingkat as $row)
                                @php
                                    $pct = $totalPerTingkat > 0 ? round($row->total / $totalPerTingkat * 100) : 0;
                                    $colors = ['bg-primary','bg-info','bg-success','bg-warning','bg-danger','bg-secondary','bg-primary'];
                                    $color  = $colors[$loop->index % count($colors)];
                                @endphp
                                <div class="d-flex align-items-center gap-2">
                                    <span class="fw-semibold text-muted" style="font-size:10px;min-width:20px;">{{ $row->grade_name }}</span>
                                    <div class="progress flex-grow-1" style="height:8px;">
                                        <div class="progress-bar {{ $color }}" style="width:{{ $pct }}%"></div>
                                    </div>
                                    <span class="fw-bold" style="font-size:10px;min-width:28px;text-align:right;">{{ $row->total }}</span>
                                    <span class="text-muted" style="font-size:10px;min-width:32px;">{{ $pct }}%</span>
                                </div>
                            @empty
                                <small class="text-muted">Belum ada data rombel aktif.</small>
                            @endforelse
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-sm-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a href="{{ route('user.students.import-form', ['userId' => $userId]) }}" class="btn btn-outline-secondary">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" class="row g-2 align-items-end">
                        ...
                        <div class="{{ $isFilteredByClass ? 'col-md-8' : 'col-md-6' }}">
                            <input type="text" name="search" class="form-control" placeholder="Cari nama, NISN, NIS, NIK..." value="{{ request('search') }}">
                        </div>
        ...
        <div class="modal-content">
            ...
            <form method="GET" id="filterFormModal">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="alumni_filter" class="form-select">
                                ...
                                <option value="alumni" {{ request('alumni_filter') === 'alumni' ? 'selected' : '' }}>Alumni</option>
                                <option value="non_alumni" {{ request('alumni_filter') === 'non_alumni' ? 'selected' : '' }}>Non Alumni</option>
                            </select>
                        </div>
                        @endif
                    </div>
                ...
                <div class="modal-footer">
                    <a href="{{ route('user.students.index', ['userId' => $userId]) }}" class="btn btn-light">Reset</a>
                    <button type="submit" class="btn btn-primary">
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item"
                                                           href="{{ route('user.student-move.index', ['userId' => $userId, 'study_group_id' => $studyGroup->id]) }}">
                                                            <i class="ri-arrow-left-right-line me-2 text-primary"></i>Pindahkan
                                                        </a>
                                                    </li>
                                                    @endif
...
function toggleStudent(id) {
    ...
    else { selectedIds.delete(id); document.getElementById('selectAllStudents').checked = false; }
    updateBulkUI();
}
...
function toggleSelectAll(el) {
    if (el.checked) {
        ...
        document.querySelectorAll('.student-checkbox').forEach(function (c) { c.checked = true; });
    } else {
        displayedStudents.forEach(function (s) { selectedIds.delete(s.id); });
        document.querySelectorAll('.student-checkbox').forEach(function (c) { c.checked = false; });
    }
    updateBulkUI();
}
