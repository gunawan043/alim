<!-- Perpustakaan Sidebar -->
@php
$currentRoute = request()->route() ? request()->route()->getName() : '';
$currentUser = auth()->user();
$userId = $currentUser->id;

if (! function_exists('isActivePerpus')) {
function isActivePerpus($routeName, $pattern) {
...
<li class="menu-title"><span>Perpustakaan</span></li>

{{-- Koleksi Buku — akan diaktifkan setelah controller dibuat --}}
<li class="nav-item">
...
<li class="nav-item">
    <a class="nav-link menu-link{{ isActivePerpus($currentRoute, 'user.students.') ? ' active' : '' }}"
       href="{{ route('user.students.index', ['userId' => $userId]) }}">
        <i class="ri-team-line"></i>
        <span>Data Peminjam (Santri)</span>
    </a>
</li>

<li class="nav-item">
    <a class="nav-link menu-link{{ isActivePerpus($currentRoute, 'user.gtk.') ? ' active' : '' }}"
