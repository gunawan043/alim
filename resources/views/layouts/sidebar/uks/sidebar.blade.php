...
@php
    $uksActive = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.');
    $uksHealthCheckActive  = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'health-checkups.index');
    $uksImmunActive        = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'immunizations.index');
    $uksPermitActive       = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'health-permits.index');
    $uksMetricsActive      = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'health-metrics.index');
    $uksCounselActive      = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'counseling-records.index');
    $uksMedicineActive     = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'medicine-inventory.index')
                            || ($isActiveFn ?? 'isActiveTU')($currentRoute, 'medicine-logs.index');
    $uksReferralActive     = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'facility-referrals.index');
    $uksSanitationActive   = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'sanitation-inspections.index');
    $uksGtkHealthActive    = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.gtk-health');
    $uksStudentHealthActive = ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.student-health');
@endphp
...
<li class="nav-item">
    ...
    <div class="collapse menu-dropdown{{ $uksActive ? ' show' : '' }}" id="uks">
        <ul class="nav nav-sm flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link{{ ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.medicine-inventory') ? ' active' : '' }}"
                   href="{{ route('user.uks.medicine-inventory.index', ['userId' => $userId]) }}"
                   style="font-size:0.85rem">Stok Obat</a>
            </li>
            <li class="nav-item">
                <a class="nav-link{{ ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.medicine-logs') ? ' active' : '' }}"
            ...
            <li class="nav-item">
                <a class="nav-link{{ ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.treatment-status') || ($isActiveFn ?? 'isActiveTU')($currentRoute, 'user.uks.patients') ? ' active' : '' }}"
