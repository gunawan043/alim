...
@section('title') Cuti & Izin GTK @endsection
...
<div class="page-header-card d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-dark mb-1" style="font-size:1.1rem">Cuti & Izin GTK</h4>
            <p class="mb-0 text-muted" style="font-size:.8rem">Daftar pengajuan cuti dan izin seluruh GTK</p>
        </div>
    </div>
...
<div class="filter-bar rounded-2 border p-3 mb-3 no-print">
    <form method="GET" action="{{ route('user.kehadiran.cuti-izin', $userId) }}" class="row g-2 align-items-end">
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <label class="form-label mb-0" style="font-size:.8rem">Jenis</label>
        ...
        <div class="col-md-3 d-flex align-items-end gap-1">
            ...
            <a href="{{ route('user.kehadiran.cuti-izin', $userId) }}" class="btn btn-light btn-sm"><i class="ri-reset-right-line me-1"></i>Reset</a>
        </div>
    </form>
</div>
...
<div class="card">
    <div class="card-header border-bottom-dashed d-flex align-items-center justify-content-between">
        <h5 class="card-title mb-0"><i class="ri-table-2 text-primary me-1"></i> Daftar Cuti & Izin GTK</h5>
    </div>
        ...
        <table class="table table-hover align-middle table-freeze">
            <thead>
                <tr>
                    ...
                    <th class="bg-light text-center">Durasi</th>
            ...
            <tbody>
                @forelse($cutiIzinList ?? [] as $item)
                    <tr>
                        ...
                        <td class="text-center">{{ $item['durasi'] ?? 0 }} <span class="text-muted small">hari</span></td>
                        <td><span class="small text-muted">{{ isset($item['alasan']) && strlen($item['alasan']) > 40 ? substr($item['alasan'], 0, 40).'...' : ($item['alasan'] ?? '-') }}</span></td>
                        ...
                        <td class="text-center no-print">
                            <a href="{{ route('user.kehadiran.cuti-izin', $userId) }}" class="btn btn-sm btn-light"><i class="ri-eye-line"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            ...
                            <p class="text-muted mb-0 small">Data cuti dan izin GTK akan muncul di sini</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if(isset($cutiIzinList) && method_exists($cutiIzinList, 'hasPages') && $cutiIzinList->hasPages())
        <div class="card-footer bg-white py-2 d-flex justify-content-between align-items-center no-print">
            <span class="text-muted small">Menampilkan {{ $cutiIzinList->firstItem() ?? 0 }} - {{ $cutiIzinList->lastItem() ?? 0 }} dari {{ $cutiIzinList->total() }} data</span>
            <nav>{{ $cutiIzinList->appends(request()->query())->links() }}</nav>
        </div>
    @endif
</div>
@endsection