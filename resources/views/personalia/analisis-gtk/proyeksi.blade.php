{{-- Analisis GTK: Proyeksi SDM --}}
@extends('layouts.master')
@section('title') Proyeksi SDM GTK @endsection
...
<div class="page-header-card d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-dark mb-1" style="font-size:1.1rem">Proyeksi SDM</h4>
            <p class="mb-0 text-muted" style="font-size:.8rem">Proyeksi kebutuhan dan ketersediaan GTK untuk perencanaan strategis</p>
        </div>
    </div>
                ...
                <div class="d-flex align-items-center gap-3">
                    <div class="avatar-sm flex-shrink-0">
                        <span class="avatar-title rounded-3 fs-2" style="background:#6366f118;">
                            <i class="ri-add-circle-line" style="color:#4f46e5;"></i>
                        </span>
                    </div>
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Kebutuhan Guru</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $kebutuhanGuru ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="d-flex align-items-center gap-3">
                    <div class="avatar-sm flex-shrink-0">
                        <span class="avatar-title bg-danger-subtle rounded-3 fs-2">
                            <i class="ri-user-follow-line text-danger"></i>
                        </span>
                    </div>
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">GTK Pensiun</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $gtkPensiun ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #16a34a;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Rekrutmen Needed</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $rekrutmenNeeded ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #d97706;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">GTK Aktif</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $gtkAktif ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
...
<div class="filter-bar rounded-2 border p-3 mb-3 no-print">
    <form method="GET" action="{{ route('user.analisis-gtk.proyeksi', $userId) }}" class="row g-2 align-items-end">
        <div class="col-md-3">
            <label class="form-label mb-0" style="font-size:.8rem">Tahun Proyeksi</label>
        ...
        <div class="col-md-3 d-flex align-items-end gap-1">
            ...
            <a href="{{ route('user.analisis-gtk.proyeksi', $userId) }}" class="btn btn-light btn-sm"><i class="ri-reset-right-line me-1"></i>Reset</a>
        </div>
...
<div class="card">
    <div class="card-header border-bottom-dashed d-flex align-items-center justify-content-between">
        <h5 class="card-title mb-0"><i class="ri-table-2 text-primary me-1"></i> Proyeksi Kebutuhan GTK</h5>
    </div>
        ...
        <table class="table table-hover align-middle table-freeze">
            <thead>
                <tr>
                    ...
                    <th class="bg-light text-center">GTK Saat Ini</th>
                    <th class="bg-light text-center">Kebutuhan</th>
                    <th class="bg-light text-center">Aksioma</th>
                    <th class="bg-light text-center">Sisa / Lebih</th>
                    <th class="bg-light text-center">Rekrutmen</th>
                    <th class="bg-light text-center">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse($proyeksiList ?? [] as $item)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td class="fw-medium">{{ $item['kelas_mapel'] ?? '-' }}</td>
                        <td class="text-center">{{ $item['gtk_saat_ini'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['kebutuhan'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['aksioma'] ?? 0 }}</td>
                        <td class="text-center">
                            @php $selisih = ($item['gtk_saat_ini'] ?? 0) - ($item['kebutuhan'] ?? 0); @endphp
                            @if($selisih >= 0)
                                <span class="badge bg-success-subtle text-success border border-success">+{{ $selisih }}</span>
                            @else
                                <span class="badge bg-danger-subtle text-danger border border-danger">{{ $selisih }}</span>
                            @endif
                        </td>
                        <td class="text-center">
                            @if(($item['kebutuhan'] ?? 0) > ($item['gtk_saat_ini'] ?? 0))
                                <span class="badge bg-primary-subtle text-primary border border-primary">{{ max(0, -$selisih) }} guru</span>
                            @else
                                <span class="text-muted small">-</span>
                            @endif
                        </td>
                        <td class="text-center no-print">
                            <a href="{{ route('user.analisis-gtk.proyeksi', $userId) }}" class="btn btn-sm btn-light"><i class="ri-eye-line"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            <div style="color:#6366f1;opacity:.4"><i class="ri-line-chart-line" style="font-size:3rem"></i></div>
                            <h5 class="mt-2 fw-semibold">Belum ada data</h5>
                            <p class="text-muted mb-0 small">Data proyeksi SDM akan muncul di sini</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if(isset($proyeksiList) && method_exists($proyeksiList, 'hasPages') && $proyeksiList->hasPages())
        <div class="card-footer bg-white py-2 d-flex justify-content-between align-items-center no-print">
            <span class="text-muted small">Menampilkan {{ $proyeksiList->firstItem() ?? 0 }} - {{ $proyeksiList->lastItem() ?? 0 }} dari {{ $proyeksiList->total() }} data</span>
            <nav>{{ $proyeksiList->appends(request()->query())->links() }}</nav>
        </div>
    @endif
</div>
@endsection
