{{-- Kalender Kegiatan: Kontrak --}}
@extends('layouts.master')
@section('title') Kalender Kontrak GTK @endsection
...
<div class="page-header-card d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-dark mb-1" style="font-size:1.1rem">Kalender Kontrak</h4>
            <p class="mb-0 text-muted" style="font-size:.8rem">Timeline dan pengingat jatuh tempo kontrak kerja GTK</p>
        </div>
    </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #16a34a;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Total Kontrak</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $totalKontrak ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #dc2626;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Akan Habis</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $akanHabis ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #16a34a;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Aktif</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $kontrakAktif ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #d97706;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Dijualkan</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $perpanjang ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
...
<div class="filter-bar rounded-2 border p-3 mb-3 no-print">
    <form method="GET" action="{{ route('user.kalender-kegiatan.kontrak', $userId) }}" class="row g-2 align-items-end">
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <label class="form-label mb-0" style="font-size:.8rem">Status Kontrak</label>
        ...
        <div class="col-md-3 d-flex align-items-end gap-1">
            ...
            <a href="{{ route('user.kalender-kegiatan.kontrak', $userId) }}" class="btn btn-light btn-sm"><i class="ri-reset-right-line me-1"></i>Reset</a>
        </div>
...
<div class="card">
    <div class="card-header border-bottom-dashed d-flex align-items-center justify-content-between">
        <h5 class="card-title mb-0"><i class="ri-table-2 text-primary me-1"></i> Timeline Kontrak Kerja GTK</h5>
    </div>
                ...
                @forelse($kontrakList ?? [] as $item)
                    <tr>
                        ...
                        <td class="text-center">
                            @php $sisa = $item['sisa_hari'] ?? 0; @endphp
                            @if($sisa <= 0)
                                <span class="badge bg-danger-subtle text-danger border border-danger">Habis</span>
                            @elseif($sisa <= 30)
                                <span class="badge bg-warning-subtle text-warning border border-warning">{{ $sisa }} hari</span>
                            @else
                                <span class="badge bg-success-subtle text-success border border-success">{{ $sisa }} hari</span>
                            @endif
                        </td>
                        <td class="text-center">
                            ...
                            @elseif($status == 'akan_habis')
                                <span class="badge bg-warning bg-opacity-10 text-warning border border-warning">Akan Habis</span>
                            @else
                                <span class="badge bg-danger bg-opacity-10 text-danger border border-danger">Habis</span>
                            @endif
                        </td>
                        <td class="text-center no-print">
                            <a href="{{ route('user.kalender-kegiatan.kontrak', $userId) }}" class="btn btn-sm btn-light"><i class="ri-eye-line"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            <div style="color:#22c55e;opacity:.4"><i class="ri-file-paper-2-line" style="font-size:3rem"></i></div>
                            <h5 class="mt-2 fw-semibold">Belum ada data</h5>
                            <p class="text-muted mb-0 small">Data kontrak GTK akan muncul di sini</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if(isset($kontrakList) && method_exists($kontrakList, 'hasPages') && $kontrakList->hasPages())
        <div class="card-footer bg-white py-2 d-flex justify-content-between align-items-center no-print">
            <span class="text-muted small">Menampilkan {{ $kontrakList->firstItem() ?? 0 }} - {{ $kontrakList->lastItem() ?? 0 }} dari {{ $kontrakList->total() }} data</span>
            <nav>{{ $kontrakList->appends(request()->query())->links() }}</nav>
        </div>
    @endif
</div>
@endsection