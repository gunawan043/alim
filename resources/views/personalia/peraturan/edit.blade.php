...
@section('title') Edit Peraturan @endsection
...
@component('components.breadcrumb')
    ...
    @slot('title') Edit Peraturan @endslot
@endcomponent

<div class="page-header-card d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-semibold mb-1">Edit Peraturan</h5>
        <p class="text-muted mb-0" style="font-size:.85rem">Perbarui dokumen peraturan yang sudah ada.</p>
    </div>
...
<form method="POST" action="{{ route('user.ats.peraturan.update', ['userId' => $userId, 'id' => $dokumen->id]) }}" class="row" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="col-lg-8">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="kategori_id" class="form-select">
                            ...
                            @foreach($kategoris as $kat)
                                <option value="{{ $kat->id }}" {{ old('kategori_id', $dokumen->kategori_id) == $kat->id ? 'selected' : '' }}>{{ $kat->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Nomor Peraturan</label>
                        <input type="text" name="nomor" class="form-control" placeholder="Contoh: SK-001/2024" value="{{ old('nomor', $dokumen->nomor) }}">
                    </div>
                </div>
                <div class="row g-3 mt-2">
                    <div class="col-md-6">
                        <label class="form-label">Tanggal Ditetapkan</label>
                        <input type="date" name="tanggal_ditetapkan" class="form-control" value="{{ old('tanggal_ditetapkan', $dokumen->tanggal_ditetapkan?->format('Y-m-d')) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Tanggal Berlaku</label>
                        <input type="date" name="tanggal_berlaku" class="form-control" value="{{ old('tanggal_berlaku', $dokumen->tanggal_berlaku?->format('Y-m-d')) }}">
                    </div>
                </div>
                <div class="mb-3 mt-2">
                    <label class="form-label">Versi</label>
                    <input type="text" name="versi" class="form-control" placeholder="Contoh: 1.0" value="{{ old('versi', $dokumen->versi) }}">
                </div>
                <div class="mb-3">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="diarsipkan" {{ old('status', $dokumen->status) == 'diarsipkan' ? 'selected' : '' }}>Diarsipkan</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Visibilitas</label>
                        <select name="visibility" class="form-select">
                            <option value="all" {{ old('visibility', $dokumen->visibility) == 'all' ? 'selected' : '' }}>Semua</option>
                            <option value="gtk" {{ old('visibility', $dokumen->visibility) == 'gtk' ? 'selected' : '' }}>GTK Saja</option>
                            <option value="personalia" {{ old('visibility', $dokumen->visibility) == 'personalia' ? 'selected' : '' }}>Personalia</option>
                            <option value="management" {{ old('visibility', $dokumen->visibility) == 'management' ? 'selected' : '' }}>Management</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center gap-2 p-2 border rounded">
                    ...
                    <span class="small text-truncate flex-grow-1">{{ $dokumen->file_path }}</span>
        ...
        <div class="d-flex gap-2 justify-content-end">
            ...
            <button type="submit" class="btn btn-primary"><i class="ri-save-line me-1"></i> Perbarui Peraturan</button>
        </div>
    </div>
</form>
@endsection