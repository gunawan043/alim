...
@section('title') Penilaian Kepribadian @endsection
...
<div class="page-header-card d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div class="d-flex align-items-center gap-3">
        <div style="width:48px;height:48px;background:#3b82f618;color:#2563eb;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0">
            <i class="ri-user-heart-line fs-4"></i>
        </div>
        <div>
            <h4 class="fw-bold text-dark mb-1" style="font-size:1.1rem">Penilaian Kepribadian GTK</h4>
            <p class="mb-0 text-muted" style="font-size:.8rem">Evaluasi sikap, karakter, dan moral GTK</p>
        </div>
    </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #16a34a;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Rata-rata Sopan Santun</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $rataSopanSantun ?? '0' }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #d97706;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Rata-rata Tanggung Jawab</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $rataTanggungJawab ?? '0' }}</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card stat-card" style="border-left:3px solid #dc2626;">
            <div class="card-body py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <p class="text-uppercase fw-medium text-muted mb-1" style="font-size:10px;letter-spacing:0.5px;">Skor Kepribadian</p>
                        <h3 class="fw-bold ff-secondary mb-0">{{ $skorKepribadian ?? '0' }}</h3>
                    </div>
                </div>
            </div>
        </div>
...
<div class="filter-bar rounded-2 border p-3 mb-3 no-print">
    <form method="GET" action="{{ route('user.rapor-gtk.kepribadian', $userId) }}" class="row g-2 align-items-end">
        <div class="col-md-3">
        ...
        <div class="col-md-3 d-flex align-items-end gap-1">
            ...
            <a href="{{ route('user.rapor-gtk.kepribadian', $userId) }}" class="btn btn-light btn-sm"><i class="ri-reset-right-line me-1"></i>Reset</a>
        </div>
...
<div class="card">
    <div class="card-header border-bottom-dashed d-flex align-items-center justify-content-between">
        <h5 class="card-title mb-0"><i class="ri-table-2 text-primary me-1"></i> Daftar Penilaian Kepribadian GTK</h5>
    </div>
        ...
        <table class="table table-hover align-middle table-freeze">
            <thead>
                <tr>
                    ...
                    <th class="bg-light text-center">Sopan Santun</th>
                    <th class="bg-light text-center">Tanggung Jawab</th>
                    <th class="bg-light text-center">Integritas</th>
            ...
            <tbody>
                @forelse($kepribadianList ?? [] as $item)
                    <tr>
                        ...
                        <td class="text-center">{{ $item['sopan_santun'] ?? '-' }}</td>
                        <td class="text-center">{{ $item['tanggung_jawab'] ?? '-' }}</td>
                        <td class="text-center">{{ $item['integritas'] ?? '-' }}</td>
                        <td class="text-center fw-bold">{{ $item['skor'] ?? '-' }}</td>
                        ...
                        <td class="text-center no-print">
                            <a href="{{ route('user.rapor-gtk.kepribadian', $userId) }}" class="btn btn-sm btn-light"><i class="ri-eye-line"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            <div style="color:#3b82f6;opacity:.4"><i class="ri-user-heart-line" style="font-size:3rem"></i></div>
                            <h5 class="mt-2 fw-semibold">Belum ada data</h5>
                            <p class="text-muted mb-0 small">Data penilaian kepribadian GTK akan muncul di sini</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if(isset($kepribadianList) && method_exists($kepribadianList, 'hasPages') && $kepribadianList->hasPages())
        <div class="card-footer bg-white py-2 d-flex justify-content-between align-items-center no-print">
            <span class="text-muted small">Menampilkan {{ $kepribadianList->firstItem() ?? 0 }} - {{ $kepribadianList->lastItem() ?? 0 }} dari {{ $kepribadianList->total() }} data</span>
            <nav>{{ $kepribadianList->appends(request()->query())->links() }}</nav>
        </div>
    @endif
</div>
@endsection
