...
@section('title') Buat Informasi @endsection
...
@section('content')
    ...
    <form method="POST"
          action="{{ route('user.asrama.posts.store', ['userId' => $userId, 'asramaUuid' => $dormitory->id]) }}"
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="undangan"   {{ old('category') == 'undangan' ? 'selected' : '' }}>Undangan</option>
                                <option value="laporan"    {{ old('category') == 'laporan' ? 'selected' : '' }}>Laporan</option>
                                <option value="darurat"    {{ old('category') == 'darurat' ? 'selected' : '' }}>Darurat</option>
                            </select>
                            @error('category')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="visibility" id="visibility"
                                ...
                                <option value="wali"      {{ old('visibility') == 'wali' ? 'selected' : '' }}>Wali Santri</option>
                                <option value="pengurus"  {{ old('visibility') == 'pengurus' ? 'selected' : '' }}>Pengurus Asrama</option>
                                <option value="umum"      {{ old('visibility') == 'umum' ? 'selected' : '' }}>Umum</option>
                            </select>
                            @error('visibility')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="d-flex gap-2 mt-3">
                    <button type="submit" class="btn btn-primary flex-grow-1">
                        <i class="ri-send-plane-line me-1"></i> Publikasikan
                    </button>
