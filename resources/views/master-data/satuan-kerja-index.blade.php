        ...
        <div class="col-lg-12">
            <div class="card" id="workUnitList">
                ...
                <div class="card-body">
                    ...
                    @include('shared._pagination', ['paginator' => $workUnits])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            workUnitForm.addEventListener('submit', async function(e) {
                ...
                const baseUrl = '/' + window.userId + '/master-data/satuan-kerja';
        ...
        async function generateCodeFromServer(divisiId, induk) {
            try {
                const response = await fetch('/' + window.userId + '/master-data/satuan-kerja/generate-code', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        async function editWorkUnit(id) {
            try {
                const response = await fetch('/' + window.userId + '/master-data/satuan-kerja/' + id, {
                    headers: {
                        'Accept': 'application/json'
                    }
                });
        ...
        async function performDelete() {
            ...
            try {
                const response = await fetch('/' + window.userId + '/master-data/satuan-kerja/' + currentDeleteId, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
        ...
        async function toggleStatus(id) {
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch('/' + window.userId + '/master-data/satuan-kerja/' + id + '/toggle-status', {
                        method: 'PATCH',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
        ...
        async function deleteMultiple() {
            ...
            document.getElementById('bulk-delete-confirm-btn').onclick = async function() {
                try {
                    const response = await fetch('/' + window.userId + '/master-data/satuan-kerja/bulk-delete', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
